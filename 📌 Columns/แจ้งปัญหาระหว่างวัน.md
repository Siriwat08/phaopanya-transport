...
| Formula version | 0 |
| Reset on edit? | No |
| Initial value | =IF(ISNOTBLANK([SM_Link]), <br>  [SM_Link].[รหัสลูกค้า], <br>  IF(ISNOTBLANK([RT_Link]), <br>    [RT_Link].[รหัสลูกค้า], <br>    "ไม่พบข้อมูล"<br>  )<br>) |
| System Defined? | No |
| Key | No |
| Part of Key? | No |
| Fixed definition? | No |
...
| Formula version | 0 |
| Reset on edit? | No |
| Initial value | =IF(ISNOTBLANK([SM_Link]), <br>  [SM_Link].[ชื่อปลายทาง], <br>  IF(ISNOTBLANK([RT_Link]), <br>    [RT_Link].[ชื่อลูกค้าที่เข้ารับ], <br>    "ไม่พบข้อมูล"<br>  )<br>) |
| System Defined? | No |
| Key | No |
| Part of Key? | No |
| Fixed definition? | No |
...
| Formula version | 0 |
| Reset on edit? | No |
| Initial value | =IF(<br>  ISBLANK([วันที่+เวลา]),<br>  "",<br>  (YEAR(DATE([วันที่+เวลา])) - IF(YEAR(DATE([วันที่+เวลา])) &gt; 2500, 543, 0))<br>  &amp; "-"<br>  &amp; TEXT(DATE([วันที่+เวลา]), "MM")<br>) |
| System Defined? | No |
| Key | No |
| Part of Key? | No |
| Fixed definition? | No |
...
| Type | Text |
| Type Qualifier | {"MaxLength":null,"MinLength":null,"LongTextFormatting":"Plain Text","IsMulticolumnKey":false,"Valid_If":null,"Error_Message_If_Invalid":null,"Show_If":null,"Required_If":null,"Editable_If":null,"Reset_If":null,"Suggested_Values":null} |
| App formula | =TEXT(<br>  LIST(<br>    IF(ISNOTBLANK([รูป1]),<br>      CONCATENATE(<br>        "https://www.appsheet.com/template/gettablefileurl?",<br>        "appName=", ENCODEURL("เผ่าปัญญาทรานสปอร์ต-915154893"),<br>        "&amp;appId=", ENCODEURL("70999aca-5a1f-4dfc-907b-417bc046f574"),<br>        "&amp;tableName=", ENCODEURL("แจ้งปัญหาระหว่างวัน"),<br>        "&amp;fileName=", ENCODEURL([รูป1])<br>      ),<br>    ""),<br>    IF(ISNOTBLANK([รูป2]),<br>      CONCATENATE(<br>        "https://www.appsheet.com/template/gettablefileurl?",<br>        "appName=", ENCODEURL("เผ่าปัญญาทรานสปอร์ต-915154893"),<br>        "&amp;appId=", ENCODEURL("70999aca-5a1f-4dfc-907b-417bc046f574"),<br>        "&amp;tableName=", ENCODEURL("แจ้งปัญหาระหว่างวัน"),<br>        "&amp;fileName=", ENCODEURL([รูป2])<br>      ),<br>    ""),<br>    IF(ISNOTBLANK([รูป3]),<br>      CONCATENATE(<br>        "https://www.appsheet.com/template/gettablefileurl?",<br>        "appName=", ENCODEURL("เผ่าปัญญาทรานสปอร์ต-915154893"),<br>        "&amp;appId=", ENCODEURL("70999aca-5a1f-4dfc-907b-417bc046f574"),<br>        "&amp;tableName=", ENCODEURL("แจ้งปัญหาระหว่างวัน"),<br>        "&amp;fileName=", ENCODEURL([รูป3])<br>      ),<br>    "")<br>  )<br>) |
| Read-Only | Yes |
| Hidden | No |
| Label | No |
| Formula version | 0 |
